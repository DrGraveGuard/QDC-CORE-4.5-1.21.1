...
public class GuiDrawFunctions {
	...
	private static final ResourceLocation BTN_FILL = BG_PURPLE_2;
	private static final ResourceLocation BTN_BORDER = BG_ORANGE_1;
	private static final ResourceLocation BTN_BORDER_ACTIVE = BG_GREEN_2;
	...
	private static final ResourceLocation TITLE_FILL = BG_ORANGE_1;
	...
	public static void drawRectangleWithBorder(GuiGraphics gg, Point windowPos, Point pos, Point size, TextureColor fillColor,
	...
	public static void drawButton(GuiGraphics gg, Point windowPos, Point pos, Point size, boolean isActive) {
		if (isActive) {
			gg.blit(BTN_BORDER_ACTIVE, windowPos.x + pos.x, windowPos.y + pos.y, 0, 0, size.x, size.y, 600, 400);
		} else {
			gg.blit(BTN_BORDER, windowPos.x + pos.x, windowPos.y + pos.y, 0, 0, size.x, size.y, 600, 400);
		}

		gg.blit(BTN_FILL, windowPos.x + pos.x + 1, windowPos.y + pos.y + 1, 0, 0, size.x - 2, size.y - 2, 600, 400);

	}

	public static void drawDisabledButton(GuiGraphics gg, Point windowPos, Point pos, Point size) {

		gg.blit(BTN_BORDER_DIASBLED, windowPos.x + pos.x, windowPos.y + pos.y, 0, 0, size.x, size.y, 600, 400);

		gg.blit(BTN_FILL_DIASBLED, windowPos.x + pos.x + 1, windowPos.y + pos.y + 1, 0, 0, size.x - 2, size.y - 2,
				600, 400);
	...
	public static void drawItemIcon(GuiGraphics gg, Font font, Point windowPos, ItemStack stack, int x, int y, int count) {
		gg.renderItem(stack, windowPos.x + x, windowPos.y + y, 3);
		gg.renderItemDecorations(font, new ItemStack(stack.getItem(), count), windowPos.x + x, windowPos.y + y, count + "");
	}
